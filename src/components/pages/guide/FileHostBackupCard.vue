<template>
  <el-card class="backupCard"
    ...
    <el-row type="flex">
      <el-col :span="18">
        <el-form label-width="100px"
          ...
          <el-form-item label="备份类型"
                        style="40%">
            <div>{{ backupType }}<span v-if="backupOperation.backupType === 1">(限速: {{fmtSizeFn(backupOperation.bwlimit*1024)}}bps)</span></div>
          </el-form-item>
          <el-form-item label="备份方向"
            ...
            <el-tooltip :content="backupOperation.sourcePath"
                        placement="top"
                        effect="dark">
              <span class="pathOverFlow">{{ backupOperation.sourcePath }}</span>
            </el-tooltip>
            <i class="el-icon-d-arrow-right"></i>
            <el-tooltip :content="backupOperation.targetPath"
                        placement="top"
                        effect="dark">
              <span class="pathOverFlow">{{ backupOperation.targetPath }}</span>
            </el-tooltip>
          </el-form-item>
      ...
      <el-col :span="6"
        ...
        <ul>
          ...
          <li>
            <h5>文件总大小</h5>
            <div>{{fmtSizeFn(backupOperation.size) || '-'}}</div>
          </li>
          <li>
            <h5>上次备份大小
            ...
            <div>{{fmtSizeFn(backupOperation.backupSize) || '-' }}</div>
          </li>
          <li>
            <h5>已备份大小
              <el-tooltip class="item" effect="dark"
            ...
            <div>{{fmtSizeFn(backupOperation.progress) || '-'}}</div>
          </li>
        </ul>
      </el-col>
    </el-row>
    <el-row v-show="isShow"
      ...
      <el-col :span="8"
        ...
        <el-card shadow="always"
                 class="childFileCard">
          <div slot="header" class="childFileHead">
          ...
          <el-row>
            <el-col :span="12">
              <el-tooltip :content="`文件备份源路径: ${backupFile.sourcePath}`"
                ...
                <div class="wordsOverFlow">
                  <i-icon name="fileSourcePath" class="backupFileInfoIcon" style="margin-right: 0.5em;"></i-icon>
                  <el-tag class="childFileTag"
                          type="warning"
                          size="mini">
            ...
            <el-col :span="12" class="backupFileState">
              <el-tooltip content="已备份大小"
                          placement="top"
                          :open-delay="300">
                <el-progress :percentage="isNaN(backupFile.percentage)?0:backupFile.percentage" class="backupFileProgress">
                </el-progress>
              </el-tooltip>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12">
              <el-tooltip :content="`备份集存放路径: ${backupFile.targetPath}`"
                          ...
                          :open-delay="300">
                <div class="wordsOverFlow">
                  <i-icon name="fileTargetPath" class="backupFileInfoIcon"></i-icon>
                  <el-tag class="childFileTag"
                          size="mini"
                          type="warning">
          ...
          <el-row>
            <el-col :span="12">
              <i-icon name="size" class="backupFileInfoIcon"></i-icon>
              <el-tooltip content="源文件总大小"
                          placement="right"
                          :open-delay="300">
                <span>{{ fmtSizeFn(backupFile.sourceSize) }}</span>
              </el-tooltip>
            </el-col>
            <el-col :span="12"
                    v-if="[1,2,3].includes(backupFile.state)"
    ...
    <transition name="el-zoom-in-center">
      <div class="balloons"
           style="bottom: 70px; left: 50%; transform: translateX(-25%)"
           v-show="['waiting', 'showResults'].includes(nextStep) && !isShow">
...
export default {
  ...
  methods: {
    ...
    formatIcon(data) {
      if (data === 0) {
        return 'waitingColor el-icon-time';
      } else if (data === 1) {
        return 'loadingColor el-icon-loading';
      } else if (data === 3) {
        return 'errorColor el-icon-error';
      } else if (data === 2) {
        return 'successColor el-icon-success';
      } else return '';
    },
    showMore() {
