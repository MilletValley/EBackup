<template>
  <section>
    <div>
      ...
      <div v-show="onGoingPlans.length === 0"
           :class="$style.noRestoreTip">
        <span>暂无操作</span>
      </div>
      <el-row v-show="onGoingPlans.length > 0"
              :gutter="20"
              style="min-height: 144px;">
        <el-col :span="8"
                v-for="item in onGoingPlans"
                :key="item.id">
...
export default {
  ...
  computed: {
    onGoingPlans() {
      return this.plans.filter(plan => plan.state === 1);
    }
  },
  components: {
    Timer,
